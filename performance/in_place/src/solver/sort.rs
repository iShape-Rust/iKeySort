...
use i_key_sort::sort::inplace::slice_one_key::OneKeyInPlaceSortSerial;
...
impl SortSolution {
    ...
    pub fn run_segments_bin_sort<S: StartEnd + Copy + Default>(segments: &[S]) -> i32 {
        ...
        for _ in 0..n {
            data.copy_from_slice(&segments);
            data.sort_by_one_key(false, |s| s.start().x);
        }
        let duration = start.elapsed().as_secs_f64() / (n as f64);
    ...
    pub fn run_segments_par_bin_sort<S: StartEnd + Copy + Default>(segments: &[S]) -> i32 {
        ...
        for _ in 0..n {
            data.copy_from_slice(&segments);
            data.sort_by_one_key(true, |s| s.start().x);
        }
        let duration = start.elapsed().as_secs_f64() / (n as f64);
    ...
    pub fn run_segments_inplace_sort<S: StartEnd + Copy + Default>(segments: &[S]) -> i32 {
        let mut data = segments.to_vec();
        let start = Instant::now();
        ...
        for _ in 0..n {
            data.copy_from_slice(&segments);
            data.ser_sort_inplace_by_one_key(|s| s.start().x);
        }
        let duration = start.elapsed().as_secs_f64() / (n as f64);

        Self::print_result("in_place_sort", duration);
